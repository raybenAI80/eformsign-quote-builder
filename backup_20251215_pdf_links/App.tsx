...
function App() {
  const { meta, items, calculation, presets, history, actions } = useQuote();
  ...
  const validation = useMemo(() => {
    ...
    if (!meta.quoteDate) metaErrors.push('견적 일자를 입력해 주세요.');
    if (!meta.contactInitials || meta.contactInitials.trim().length < 2) metaErrors.push('담당자 이니셜을 2자 이상 입력해 주세요.');
    if (!meta.issueSequence) metaErrors.push('발행 순번을 입력해 주세요.');
    if (!meta.customerName?.trim()) metaErrors.push('고객사를 입력해 주세요.');
    if (!meta.customerManager?.trim()) metaErrors.push('고객사 담당자를 입력해 주세요.');
    if (!meta.contactName?.trim()) metaErrors.push('영업 담당자 이름을 입력해 주세요.');
    if (!meta.contactEmail?.trim()) metaErrors.push('이메일을 입력해 주세요.');
    if (!meta.contactMobile?.trim()) metaErrors.push('휴대전화 번호를 입력해 주세요.');
    if (!meta.contactDirect?.trim()) metaErrors.push('직통 번호를 입력해 주세요.');

    const itemErrors: string[] = [];
    if (items.length === 0) itemErrors.push('품목을 1개 이상 추가해 주세요.');
    items.forEach((it, idx) => {
      if (!it.item?.trim()) itemErrors.push(`품목명 누락: ${idx + 1}행`);
      if (!it.qty || it.qty <= 0) itemErrors.push(`수량이 0 이하: ${idx + 1}행`);
      if (it.unitPrice == null || it.unitPrice < 0) itemErrors.push(`단가가 비었습니다: ${idx + 1}행`);
    });

    const optionErrors: string[] = [];
    if (!meta.brandingMode) optionErrors.push('브랜딩 모드를 선택해 주세요.');
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['섹션', '품목명', '수량', '단가', '금액', '할인율', '제안가', '비고'];
  ...
  const renderWorkspace = () => {
    switch (activeTab) {
      case 'basic':
        return <BasicInfoEditor meta={meta} setMeta={actions.setMeta} isEditing={isEditing} />;
      case 'items':
        return (
          <ItemEditor
            items={items}
          ...
          <div className="space-y-6">
            ...
            {!isEditing && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 shadow-sm">
                편집 모드가 꺼져 있어 값이 잠겨 있습니다. 상단 “편집 모드”를 켜면 다시 수정할 수 있습니다.
              </div>
            )}
